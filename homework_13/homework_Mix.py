...
class Window(tk.Tk):
    ...
    def clear_text(self):
        ...
        self.heightIntVar.set("")
        self.weightIntVar.set("")
    ...
    def check_value(self):
        #姓名欄位
        if self.nameStringVar.get() == "" :
            self.messageText.config(state=tk.NORMAL)
            self.messageText.insert(tk.END,'請輸入姓名\n')
            self.messageText.config(state=tk.DISABLED)
        #出生年月日欄位
        self.birthValue = self.birthStringVar.get()
        dateRegex = re.compile(r"^\d\d\d\d/\d\d/\d\d$")
        self.birthMatch = re.match(dateRegex,self.birthValue)
        if not self.birthMatch:
            error_flag = True
            self.messageText.config(state=tk.NORMAL)
            self.messageText.insert(tk.END,'生日欄位格式應為(YYYY/MM/DD)\n')
            self.messageText.config(state=tk.DISABLED)
        else:
            birth = datetime.date(int(self.birthMatch.group(1)),int(self.birthMatch.group(2)),int(self.birthMatch.group(3)))
            age = self.Age_c(birth)
            if age < 0:
                error_flag = True
                self.messageText.config(state=tk.NORMAL)
                self.messageText.insert(tk.END,'生日欄位應為過去時間\n')
                self.messageText.config(state=tk.DISABLED)
        #判斷星座
        birth_mo_day = int(self.birthMatch.group(2)), int(self.birthMatch.group(3))
        self.star_sign = ""
        ...
        if birth_mo_day[0] == 1 and birth_mo_day[1] >= 21:
            self.star_sign = "水瓶座（Aquarius）01/21~2/19"
        #身高欄位
        if not self.heightIntVar.get():
            error_flag = True
            self.messageText.config(state=tk.NORMAL)
            self.messageText.insert(tk.END,'請輸入身高\n')
            self.messageText.config(state=tk.DISABLED)
        elif not 100 < self.heightIntVar.get() < 220:
            error_flag = True
            self.messageText.config(state=tk.NORMAL)
            self.messageText.insert(tk.END,'身高範圍錯誤(100~220)\n')
            self.messageText.config(state=tk.DISABLED)
        #體重欄位
        if not self.weightIntVar.get():
            error_flag = True
            self.messageText.config(state=tk.NORMAL)
            self.messageText.insert(tk.END,'請輸入體重\n')
            self.messageText.config(state=tk.DISABLED)
        elif not 20 < self.weightIntVar.get() < 200:
            error_flag = True
            self.messageText.config(state=tk.NORMAL)
            self.messageText.insert(tk.END,'體重範圍錯誤(20~200)\n')
            self.messageText.config(state=tk.DISABLED)
        if not error_flag:
            pass
        else:
    ...
    def Age_c(self):
        ...
        birthday = datetime.date(int(self.birthMatch.group(1)), int(self.birthMatch.group(2)), int(self.birthMatch.group(3)))
    ...
    def check_value(self):
        ...
        if  birthMatch is None:            
            self.birthValue = ""
        #檢查年份
        elif datetime.datetime.today().year  - int(birthMatch.group(1)) <= 0:
            self.birthValue = ""
        #檢查月份
        elif datetime.datetime.today().month - int(birthMatch.group(2)) <= 0 :
            self.birthValue = ""
        #檢查幾號
        elif datetime.datetime.today().day - int(birthMatch.group(3)) <= 0 :
            self.birthValue = ""
    ...
    def print_message(self):
        ...
        self.messageText.config(state=tk.NORMAL)
        self.messageText.insert(tk.END, f"姓名：{self.nameValue}\n")
        self.messageText.insert(tk.END, f"生日：{self.birthValue}\n")
        self.messageText.insert(tk.END, f"年齡:{self.age}\n")
        self.messageText.insert(tk.END, f"{self.star_sign}")
        self.messageText.insert(tk.END, f"BMI：{self.bmi:.2f}\n")
        self.messageText.insert(tk.END, f"BMI類別：{self.BMI_message}\n")
        self.messageText.config(state=tk.DISABLED)
